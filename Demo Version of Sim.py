...
t = 0                 # time in seconds
TimeStep = 0.001          # Timefactor (seconds per cycle) lower=more accurate
CounterLimit = 100    # How many iterations the counter will go up to before triggering and reseting
...
while stop == 0:
  ...
  # Accumulate records every X itterations
  Counter = (Counter + 1)
...
WindowResolution = pygame.display.set_mode((1600,900))
pygame.display.set_caption("bIlL nYe ThE sCiEnCe FrY")
...
def ScoreBoard(s):
    ...
    text = font.render ('Score: '+str(s),Tr`ue, (0,0,0))

    WindowResolution.blit(text, (0,0))
...
while not IsClosed:
    ...
    WindowResolution.fill(white)
    WindowResolution.blit(Bill)
